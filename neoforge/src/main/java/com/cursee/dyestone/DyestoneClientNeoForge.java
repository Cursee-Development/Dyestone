...
public class DyestoneClientNeoForge {
    ...
    public static class ModClientBusEvents {
        ...
        public static void onRegisterColorHandlers(final RegisterColorHandlersEvent.Block event) {
            ...
            event.register((state, tintGetter, pos, i) -> 0xFFFF0000, ModBlocks.DYED_RED_REDSTONE_WIRE);
            ...
            event.register((blockState, blockAndTintGetter, blockPos, i) -> 0xFF0000FF, ModBlocks.DYED_BLUE_REDSTONE_WIRE);
    ...
    private static final Vec3[] ORIGINAL_COLORS = Util.make(new Vec3[16], vec3Array -> {
    ...
    private static final Vec3[] GREEN_COLORS = Util.make(new Vec3[16], vec3Array -> {
        for (int power = 0; power <= 15; ++power) {
            float inRangePower = power / 15.0f;
